-- снова переписываю, т.к. у меня все поля в text, а Кристина на уроке сказала, что constraint check та еще нагрузка, поэтому переделываю на varchar
-- здесь же доделываю комментарии, в том числе, и по 4 заданию
...
(
    ...
    CONSTRAINT public_discount_type_type_ui UNIQUE (type)
);
...
COMMENT ON CONSTRAINT public_discount_type_type_ui ON public.discount_type IS 'Тип скидки должен быть уникальным';
...
CREATE INDEX public_discount_discount_type_index ON public.discount (discount_type_id);
CREATE INDEX public_discount_product_index ON public.discount (product_id);
CREATE INDEX public_discount_sku_index ON public.discount (sku_id);
...
COMMENT ON COLUMN public.discount.discount_type_id IS 'Ссылка на тип скидки';
COMMENT ON COLUMN public.discount.sku_id IS 'Ссылка на ТП';
COMMENT ON COLUMN public.discount.product_id IS 'Ссылка на товар';
COMMENT ON COLUMN public.discount.discount_start IS 'Дата и время начала действия скидки';
COMMENT ON COLUMN public.discount.discount_end IS 'Дата и время окончания действия скидки';
COMMENT ON COLUMN public.discount.is_active IS 'Статус - активная ли';

COMMENT ON CONSTRAINT public_discount_name_discount_start_ui ON public.discount IS 'Уникальный ключ для имени скидки и даты ее начала';
COMMENT ON CONSTRAINT public_discount_discount_type_fk ON public.discount IS 'Внешний ключ на тип скидки';
COMMENT ON CONSTRAINT public_discount_sku_fk ON public.discount IS 'Внешний ключ на ТП';
COMMENT ON CONSTRAINT public_discount_product_fk ON public.discount IS 'Внешний ключ на продукт';
COMMENT ON CONSTRAINT public_discount_value_check ON public.discount IS 'Ограничение на значение скидки - не может быть меньше ноля';
COMMENT ON CONSTRAINT public_discount_relationship_check ON public.discount IS 'Ограничение - скидка может быть либо на ТП, либо на товар, но не сразу на то и другое';

COMMENT ON INDEX public_discount_discount_type_index IS 'Индекс на внешний ключ - тип скидки';
COMMENT ON INDEX public_discount_product_index IS 'Индекс на внешний ключ - на продукт';
COMMENT ON INDEX public_discount_sku_index IS 'Индекс на внешний ключ - на ТП';
...
ALTER TABLE public.customer
    ADD CONSTRAINT public_customer_relationship_check CHECK (
        CASE
            WHEN type = 'физлицо' AND customer_address_id NOTNULL THEN TRUE
            WHEN type = 'юрлицо' AND legal_person_id NOTNULL THEN TRUE
            ELSE FALSE
            END
        );
...
CREATE INDEX public_product_sku2order_sku_index ON public.product_sku2order (sku_id);
CREATE INDEX public_product_sku2order_product_index ON public.product_sku2order (product_id);
CREATE INDEX public_product_sku2order_shop_order_index ON public.product_sku2order (shop_order_id);
...
COMMENT ON COLUMN public.product_sku2order.sku_id IS 'Ссылка на ТП';
COMMENT ON COLUMN public.product_sku2order.product_id IS 'Ссылка на товар';
COMMENT ON COLUMN public.product_sku2order.shop_order_id IS 'Ссылка id заказа';
COMMENT ON COLUMN public.product_sku2order.quantity IS 'Поле количество на каждый артикул-товар';

COMMENT ON CONSTRAINT public_sku2order_sku_order_ui ON public.product_sku2order IS 'Уникальность определятся по заказу, ТП, товару';
COMMENT ON CONSTRAINT public_product_sku2order_sku_fk ON public.product_sku2order IS 'Внешний ключ на ТП';
COMMENT ON CONSTRAINT public_product_sku2order_product_fk ON public.product_sku2order IS 'Внешний ключ на товар';
COMMENT ON CONSTRAINT public_product_sku2order_order_fk ON public.product_sku2order IS 'Внешний ключ на заказ';
COMMENT ON CONSTRAINT public_product_sku2order_relationship_check ON public.product_sku2order IS 'Ограничение, может быть ссылка либо на ТП, либо на заказ';

COMMENT ON INDEX public_product_sku2order_sku_index IS 'Индекс на внешний ключ - на ТП';
COMMENT ON INDEX public_product_sku2order_product_index IS 'Индекс на внешний ключ - на товар';
COMMENT ON INDEX public_product_sku2order_shop_order_index IS 'Индекс на внешний ключ - на заказ';
